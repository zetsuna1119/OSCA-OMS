...
<div class="search-container">
  ...
  <label for="search">Select Date:</label>
    <input type="date" name="search" style="width: 125px; margin-left: 5px;" placeholder="Search by Creation Date...">
    <label for="filter">Select Barangay:</label>
    <input style="margin-left: 5px;"  type="text" id="filter" list="filterOptions" name="filter" value="<?= $filter ?>">
    <datalist id="filterOptions">
    <option value="Abachanan">
    <option value="Anibongan">
    <option value="Bugsok">
    <option value="Cahayag">
    <option value="Canlangit">
    <option value="Canta-ub">
    <option value="Casilay">
    <option value="Danicop">
    <option value="Dusita">
    <option value="La Union">
    <option value="Lataban">
    <option value="Magsaysay">
    <option value="Matin-ao">
    <option value="Nan-od">
    <option value="Poblacion">
    <option value="Salvador">
    <option value="San Augustin">
    <option value="San Isidro">
    <option value="San Jose">
    <option value="San Juan">
    <option value="Santa Cruz">
    <option value="Villa Garcia">
  </datalist>
...
$$search = isset($_GET['search']) ? $_GET['search'] : '';
$filter = isset($_GET['filter']) ? $_GET['filter'] : '';
$stmt = $mysqli->prepare("SELECT `id`, `StuID`, `SurName`, `FirstName`, `MiddleName`, `Barangay`, `Gender`, `Age`, `DateofAdmission` FROM `tbl_pension` WHERE (`CreationDate` LIKE CONCAT('%', ?, '%') OR `FirstName` LIKE CONCAT('%', ?, '%')) AND `Barangay` LIKE CONCAT('%', ?, '%') ORDER BY `SurName`, `FirstName`, `MiddleName` ASC LIMIT $offset, $total_records_per_page");
$stmt->bind_param("sss", $search, $search, $filter);
...
<?php } ?>
  ...
  <a class="page-link <?= ($page_no <= 1) ? 'disabled' : ''; ?> " <?= ($page_no > 1) ? 'href=?page_no=' . $previous_page . '&filter=' . $filter : ''; ?>>Previous</a>
    ...
    <a class="page-link <?= ($page_no >= $total_no_of_pages) ? 'disabled' : ''; ?>" <?= ($page_no < $total_no_of_pages) ? 'href=?page_no=' . $next_page . '&filter=' . $filter : ''; ?>>Next</a>
