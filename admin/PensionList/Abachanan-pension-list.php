...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Ln();
        $this->Cell(0, 5, 'BARANGAY ABACHANAN', 0, 0, 'C');
        $this->Ln();
        $this->Cell(0, 5, 'Date of Pay-out: ____________', 0, 0, 'L');
        $this->Cell(0, 5, 'SDO: ______________', 0, 0, 'R');
        $this->Ln();
        $this->SetFont('Arial', '', 9);
        $this->Cell(8, 10, 'No.', 1, 0, 'C');
        // $this->Cell(25, 10, 'BARANGAY', 1, 0, 'C');
...
$query = mysqli_query($con, "SELECT * FROM tblsenior  WHERE Barangay = 'Abachanan' ORDER BY `SurName`, `FirstName`, `MiddleName` ASC");
while ($data = mysqli_fetch_array($query)) {
    ...
    // if($pdf->GetStringWidth(strtoupper($data['Barangay'])) > 25) {
    //     $pdf->SetFont('Arial','',8);
    //     $pdf->Cell(25, 6, (strtoupper($data['Barangay'])), 1, 0);
    //     $pdf->SetFont('Arial', '', 10);
    // } else {
    // $pdf->Cell(25, 6, strtoupper($data['Barangay']), 1, 0);
    // }
